...
- API 核對：已完整讀取已安裝 pi `docs/extensions.md`、`docs/session-format.md`、`docs/sessions.md`；工具直接觀測共用 Game 結果，`session_start`／`session_tree`／`session_shutdown` 重設快照；generation 防止舊分支延遲操作回覆寫入新分支。watch 僅使觀測失效，不在背景擷取或換裝。
