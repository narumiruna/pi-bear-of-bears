...
| `/inventory` | 顯示背包、物品編號及裝備控制項；超過單頁時會省略較弱項目並提示分頁。 |
| `/inventory all` | 顯示完整背包；可能拆成連續多則訊息。配裝評分應優先使用此指令，並確認「全列」宣告數與解析列數一致。 |
| `/inspect 編號` | 檢視物品描述、已列屬性及可能的比較資訊；目前已確認 inspect 會漏列背包中的屬性，完整技能／被動語意尚未驗證。標準裝備應與完整背包列交叉核對；特殊效果或未知格式不得猜測。背包也標示可依物品名稱查詢。 |
| `/idlestatus` | 顯示預估掛機進度，不停止掛機模式，也不領取待結算獎勵。 |

機器人明確表示 `/look`、`/status`、`/inventory`、`/who` 及 `/idlestatus` 不會中斷掛機模式；`/inventory all` 是已觀察到的完整清單變體，配裝流程仍須以新回覆確認。
不要將這項保證推廣到所有其他指令。
...
| `/sellall 稀有` | 允許的批次販售指令；依說明販售稀有及更低稀有度裝備。須先取得明確授權並確認預覽。 |
| `/sellall 史詩` | 僅在即時狀態確認目前角色尚未完成二轉時允許；須取得對史詩及更低稀有度裝備的明確販售授權並確認預覽。角色已二轉或二轉狀態不明時禁止使用。 |
| `/sellall 全部` | **禁止使用**；不得以任何理由執行。 |
| `/forge` | 顯示目前角色可進階王裝的編號、穿戴狀態、現有王之精魄、所需精魄與金幣；可直接計算各候選進化的材料缺口，僅查看不等於進階或合成新裝。需精魄時依當前 `/boss`、`/help` 及實際結算研究安全可取得的來源，不因合成新裝入口未明而略過。 |
| `/forge 編號` | 依剛取得的 `/forge` 選單進階指定王裝（base →·極 →·神），可能消耗精魄與金幣、直接穿上新裝。執行前停止掛機並結算，重新核對角色、完整背包、穿戴、費用和角色自身收益；執行後查背包、屬性與網站裝備力，不把數值增幅直接當成網站分數增幅。657746 選單明列此語法；657719 狂戰魔刃升極後網站身上裝備力未變。 |
| `/market` | 開啟角色建立回覆中介紹的熊熊交易所。尚未確認交易語法及交易行為；提出任何交易前先檢視目前選單。 |

允許的 `/sellall` 仍須由遊戲端預覽確認會保護已裝備物品、各欄位最強物品、鎖定物品與消耗品。
`/sellall 稀有` 會保留史詩及傳說裝備；`/sellall 史詩` 僅限尚未二轉的目前角色使用，並應依預覽確認會保留傳說裝備。
禁止 `/sellall 全部` 及其他變體。確認將販售哪些物品並取得對應稀有度的核准，不要只依賴指令名稱。
